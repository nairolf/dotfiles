...
php_install() {
  ...
  # add deprecated php versions
  brew tap exolnet/homebrew-deprecated || true

  # Install PHP versions.
  for PHP_VER in "5.6" "7.0" "7.1" "7.2" "7.3"; do
    brew install "php@${PHP_VER}" || true
    brew link --force --overwrite "php@${PHP_VER}" || true
    echo "date.timezone = Europe/Berlin" > "$(brew --prefix)/etc/php/${PHP_VER}/conf.d/date.ini"
